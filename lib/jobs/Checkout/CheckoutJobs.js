...
class CheckoutJobs extends JobsHandler_1.JobsHandler {
    constructor(localStorageHandler, apolloClientManager) {
        ...
        this.createCheckout = ({ email, lines, channel, shippingAddress, selectedShippingAddressId, billingAddress, selectedBillingAddressId, note }) => __awaiter(this, void 0, void 0, function* () {
            const cleanedLines = lines.filter(line => line.quantity !== 0);
            const { data, error } = yield this.apolloClientManager.createCheckout(email, cleanedLines, channel, shippingAddress, billingAddress, note);
            if (error) {
                /**
        ...
        this.updateNote = ({ token, note }) => __awaiter(this, void 0, void 0, function* () {
            const { error } = yield this.apolloClientManager.updateNote(token, note);
            if (error) {
                return {
                    dataError: {
                        error,
                        type: types_1.DataErrorCheckoutTypes.UPDATE_NOTE,
                    },
                };
            }
            return {};
